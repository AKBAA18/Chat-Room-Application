...
(TCP socket) 
 Description : This file is a server side program which accept clients based on Authentication
               and stores the client side chat history in a file.On conclusion, this file mainly focuss on
               IPv4 TCP socket based communication. 
...
void* handle_client(void* arg) {
    ...
    while (1) {
        if ((receiver_descriptor = accept(server_descriptor, (struct sockaddr*)&client, &length)) == -1) {
            perror("accept");
            exit(1);
        }
        ...
        if (strcmp(passwd, userpasswd) == 0 && (strcmp(username, user1) == 0 || strcmp(username, user2) == 0 || strcmp(username, user3) == 0 || strcmp(username, user4) == 0)) {
            ...
            while (a) {
                ...
                //If client enters bye on next iteration for no mess the bytes_read will be 0  
                if(bytes_read==0)
                {
    ...
    if(strcmp(message,"Bye")==0)
                {
                  printf("Client need to disconnect \n");
                  strcpy(status ,"Bye" );
                  a=0;
                  break;
    }
    ...
    {
      //this is for adding the new line 
